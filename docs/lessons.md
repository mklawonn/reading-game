...
**One screen per tier, dimensionally.** The home screen is a **street**
(ABC-style): one hero building per world (generated by
`content/gen-world-art.py`), paged horizontally with parallax clouds and a
distant skyline, the sky recoloring per world — locked worlds stand in
greyscale down the road. Tapping a building walks **inside**: wood-panel
interiors where rooms, then lesson nodes, climb a bottom-up winding dotted
trail as chunky pseudo-3D coins (`RaisedNode` — face on a darker rim,
presses down on touch; gold when done). Transitions are a gentle
fade-and-grow (`pushImmersive`) so entering somewhere feels like arriving.
The big Play button on every screen skips the walking and jumps straight
into the next lesson — exploration is optional, progress is one tap.
