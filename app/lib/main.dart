...
class _ReadingGameAppState extends State<ReadingGameApp> {
  ...
  // Load the Content Bank once, then build the progression engine from it.
  // (Persistence/Firestore loads into this ProgressService later.)
  late final Future<ProgressService> _progress =
      _contentService.load().then((bank) => ProgressService(bank: bank));
    ...
    return MaterialApp(
      ...
      home: FutureBuilder<ProgressService>(
        ...
        builder: (context, snapshot) {
          ...
          // Games are decoupled from progression: they only emit LearningEvents
          // via onEvent. Wiring that to progress.record is the entire coupling.
